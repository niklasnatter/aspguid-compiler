...
/**
 * Model for an input attribute element of a graphical user interface definition.
 */
public class InputAttributeDefinition {
    private static final String objectName = "@input_attribute";
    ...
    private InputAttributeDefinition(String identifier) throws InvalidIdentifierFormatException {
        JsonHelper.assertIdentifierIsValid(objectName, identifier);
        this.identifier = identifier;
    }

    /**
     * Create a new input attribute model instance with the given identifier from the given json element.
     * The model is created by extracting each property of the given json element and setting it to the
     * model.
     *
     * @param identifier identifier of the input attribute element
     * @param j          valid input attribute json element according to the gui definition language specification
     * @return model instance
     * @throws DefinitionFormatException if the given json element is not valid according to the gui definition
     *                                   language specification
     */
    public static InputAttributeDefinition fromJsonElement(String identifier, JsonElement j) throws DefinitionFormatException {
        Logger.getGlobal().info("[parsing] read input attribute element '" + identifier + "'");

        if (!j.isJsonObject()) throw new InvalidJsonObjectException(objectName);

        InputAttributeDefinition inputAttribute = new InputAttributeDefinition(identifier);
        for (Map.Entry<String, JsonElement> entry : j.getAsJsonObject().entrySet()) {
            inputAttribute.setValueByJsonProperty(entry.getKey(), entry.getValue());
        }
